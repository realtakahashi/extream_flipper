...
mod flipper_three {
    ...
    impl ContractBase for FlipperThree {
        #[ink(message)]
    ...
    impl FlipperThree {
        ...
        pub fn _circuler_flip(&mut self, mut vec_of_parameters:Vec<String>) -> Result<(), ContractBaseError> {
            ink::env::debug_println!("######################### [flipper_3] _circuler_flip is called");
            ink::env::debug_println!("######################### [flipper_3] vec_of_parameters is {:?}",vec_of_parameters);
            self.flip();
            ...
            if vec_of_parameters.len() >= 1 && vec_of_parameters[0] != "" {
                ...
                ink::env::debug_println!("######################### [flipper_3] finished");
                Ok(())
            }
        }
    }

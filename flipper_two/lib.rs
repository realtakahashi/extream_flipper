...
mod flipper_two {
    ...
    impl ContractBase for FlipperTwo {
        #[ink(message)]
    ...
    impl FlipperTwo {
        ...
        pub fn _circuler_flip(&mut self, mut vec_of_parameters:Vec<String>) -> Result<(), ContractBaseError> {
            ink::env::debug_println!("######################### [flipper_2] _circuler_flip is called");
            ink::env::debug_println!("######################### [flipper_2] vec_of_parameters is {:?}",vec_of_parameters);
            self.flip();
            ...
            if vec_of_parameters.len() >= 1 && vec_of_parameters[0] != "" {
                ...
                ink::env::debug_println!("######################### [flipper_2] to_string is {:?}",to_string);
                ink::env::debug_println!("######################### [flipper_2] param_string is {:?}",param_string);
                ...
                ink::env::debug_println!("######################### [flipper_2] finished");
                Ok(())
            }
        }
    }
